...
ylims <- list(list(c(-2.5, 2.5), c(-2, 2), c(-1.5, 1.5)), list(c(-4.5, 4.5), c(-2.5, 2.5), c(-2, 2)))
case_ranges <- list(c(0, 201), c(201, 501), c(501, Inf))#, c(401, Inf))[1:4]
n_ran <- length(case_ranges)
par(mfrow = c(2, n_ran), mar = c(3, 3, 2, 2), oma = c(4, 4, 7, 12))
# case_ranges <- list(c(0, 50), c(51, 100), c(101, 200), c(201, Inf))
...
for (n_week_ahead in 1:2) {
  ...
  for(j in 1:length(case_ranges)) {
    ...
    #ypl <- log2(ir_keep[, paste0("nt_plus_", n_week_ahead)] / ir_keep$nt_plus_0)
    cor_out <- cor.test(xpl, ypl, use = "p", me = "sp")
    cor_res[[n_week_ahead]][[case_labs[j]]] <- cor_out
    plot(xpl, ypl, main = "", xlab = "", ylab = "", xlim = c(0, 3), 
    ...
    mtext(side = 3, line = .5, text = paste("rho = ", round(cor_out$estimate, 2)), cex = .8)#, " p = ", signif(cor_out$p.value, 2)), cex = .75)
  }
}

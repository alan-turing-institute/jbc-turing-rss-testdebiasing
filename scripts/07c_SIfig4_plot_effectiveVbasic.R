...
pdf(file.path(plot_dir, "R_basicVeffective.pdf"), width = 18 * cm2in, 12 * cm2in, pointsize = 7)
...
legend(x = "center", legend = c("Effective R", "Basic R"),
...
for (ltla_curr in ltla_plot) {
  ...
  what_pl <- "R"
  ylim_use <- switch(what_pl, I = c(0, max_prev2), R = c(0, 4))
  ...
  SIR_model_results[[ltla_curr]]$R_basic <- d$M[1] * SIR_model_results[[ltla_curr]]$R /
    (d$M[1] - SIR_model_results[[ltla_curr]]$I - SIR_model_results[[ltla_curr]]$Rplus)
  ...
  if(what_pl == "R") {
    ...
    SIR_model_results[[ltla_curr]]$R_basic_quant <- t(apply(SIR_model_results[[ltla_curr]]$R_basic[, its_keep], 1, 
                                                            function(v) quantile(v, qplot, na.rm = T)))
    matbasic <- SIR_model_results[[ltla_curr]]$R_basic_quant
  }
  colc <- 1
  ...
  lines(xpl_week, matbasic[, 2], lty = 1, lwd = 4, col = col_use$imperial)
  lines(xpl_week, matbasic[, 3], lty = 1, lwd = 1, col = col_use$imperial)
  lines(xpl_week, matbasic[, 1], lty = 1, lwd = 1, col = col_use$imperial)
  annotate_months(plot_map, add_axis = T, shade_alpha = .2, for_presentation = T)
}

mtext(side = 2, outer = T, at = .25, text = "R value")

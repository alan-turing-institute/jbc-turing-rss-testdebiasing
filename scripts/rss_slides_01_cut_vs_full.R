...
del_seq <- seq(1, 6, len = n_del)#boot::logit(ilogit_del_plus_nu_seq) - nu[j]
...
log_joint_posterior <- function(delta, I, del_seq) {
...
log_p12_posterior <- function(delta, I, del_seq) {
  # approxfun(del_seq + nu[j], prior_on_nu_plus_delta(del_seq + nu[j]))(delta + nu[j]) +
    # approxfun(control$I_seq, I_logprior[j, ])(I) +
    dbinom(test_df$nt[j], size = I, prob = boot::inv.logit(delta + nu[j]), log = T) +
    dbinom(test_df$Nt[j] - test_df$nt[j], size = M_curr - I, prob = boot::inv.logit(nu[j]), log = T)
...
log_delta_cond_posterior <- function(delta, I, del_seq) {
  approxfun(del_seq + nu[j], prior_on_nu_plus_delta(del_seq + nu[j]))(delta + nu[j]) +
    dbinom(test_df$nt[j], size = I, prob = boot::inv.logit(delta + nu[j]), log = T) +
    dbinom(test_df$Nt[j] - test_df$nt[j], size = M_curr - I, prob = boot::inv.logit(nu[j]), log = T)
}
...
for (del_curr in del_seq) {
  coarse_log_marginal <- log_joint_posterior(delta = del_curr, I = I_seq_coarse, del_seq = del_seq)
  ...
  if (all(is.na(coarse_log_marginal)) || all(na.omit(coarse_log_marginal) < log_bound)) {
    ...
    fine_marginal <- log_joint_posterior(delta = del_curr, I = I_seq_fine, del_seq = del_seq)
...
for (del_curr in del_seq) {
  fine_log_marginal <- log_joint_posterior(delta = del_curr, I = I_fine, del_seq = del_seq)
...
for (I_curr in I_fine) {
  del_log_post_curr <- log_delta_cond_posterior(del_seq, I_curr, del_seq)
...
#####################################################
# Evaluate the bivariate posteriors just for P12 data

log_p12_post_I_delta_fine <- matrix(NA, length(del_seq), length(I_fine), dimnames = list(del_seq, I_fine))
for (del_curr in del_seq) {
  # fine_log_marginal <- log_joint_posterior(delta = del_curr, I = I_fine, del_seq = del_seq)
  log_p12_post_I_delta_fine[match(del_curr, del_seq), ] <- log_p12_posterior(delta = del_curr, I = I_fine, del_seq = del_seq)
}
log_p12_post_I_delta_fine[is.na(log_p12_post_I_delta_fine)] <- -Inf
p12_2d_log_posterior_unnorm <- log_p12_post_I_delta_fine - max(log_p12_post_I_delta_fine)
p12_2d_posterior_unnorm <- exp(p12_2d_log_posterior_unnorm)
p12_2d_posterior_norm <- p12_2d_posterior_unnorm / sum(p12_2d_posterior_unnorm, na.rm = T)# / pi_delta_bin_area
p12_2d_log_posterior <- log(p12_2d_posterior_norm)
...
for (I_curr in I_fine) {# I_curr <- I_fine[500]#
  coarse_log_marginal <- log_joint_posterior(delta = del_seq, I = I_curr, del_seq = del_seq)
  if (all(is.na(coarse_log_marginal)) || all(na.omit(coarse_log_marginal) < log_bound)) {
    ...
    fine_marginal <- log_joint_posterior(delta = del_seq_fine, I = I_curr, del_seq = del_seq_fine)
...
graphics.off()
plot_dir <- "C:/Users/nicho/Dropbox/Apps/Overleaf/RSS debiased prevalence slides/plots"
jpeg(paste0(plot_dir, "/cut_vs_full_posteriors.jpeg"), 9, 5, res = 750, units = "in")
par(mar = c(3, 3, 5, 5), oma = c(1, 1, 1, 1), mfrow = c(1, 2))
cexax <- 1
# plot_bd <- -20
for(plot_type in c("full", "cut")) {
  if(plot_type == "full") {
...
graphics.off()
plot_dir <- "C:/Users/nicho/Dropbox/Apps/Overleaf/RSS debiased prevalence slides/plots"
jpeg(paste0(plot_dir, "/cut_vs_full_validation.pdf"), 9, 5, res = 750, units = "in")
par(mar = c(3, 3, 5, 5), oma = c(1, 1, 1, 1), mfrow = c(1, 2))
cexax <- 1
bias_store <- list()
for (plot_type in c("full", "cut")) {
...
#####################################################
# Evaluate the bivariate posteriors just for P12 data
# nu <- boot::logit((test_df$Nt - test_df$nt)/(test_df$M * (1 - .027)))
nu <- boot::logit((test_df$Nt - test_df$nt)/(test_df$M * (1)))
...
graphics.off()
image(x = 1:n_del, y = 1:n_pi_fine_plot, z = p12_2d_log_posterior, xaxt = "n", yaxt = "n", xlab = "", ylab = "", bty = "n")
      # col = grey_pallette, bty = "n")#, zlim = c(0, 1))
mtext(side = 2, line = 2.5, text = expression(paste("% Prevalence  ", pi)), cex = cexax)
mtext(side = 1, line = 2.5, text = expression(paste("Bias parameter  ", delta)), cex = cexax)
del_labs <- pretty(del_seq)
